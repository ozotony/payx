namespace XPay.xis.pd.tx
{
    ...
    public partial class formx : Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            {
              //  Double amount = Convert.ToDouble(this.Session["einao_split_amt"]) + Convert.ToDouble(this.Session["cld_split_amt"]);
                   this.einao_split_amt = this.Session["einao_split_amt"].ToString();
              //  this.einao_split_amt = Convert.ToString(amount);
            }
            if (this.Session["cld_split_amt"] != null)
            {
                  this.cld_split_amt = this.Session["cld_split_amt"].ToString();
              //  this.cld_split_amt = "0";
            }

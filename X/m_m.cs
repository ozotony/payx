namespace XPay.X
{
    ...
    public partial class m_m : Page
    {
        ...
        protected void btnAddItem_Click(object sender, EventArgs e)
        {
            ...
            base.Response.Redirect("./m_m.aspx");
        }
        ...
        protected void btnEditItem_Click(object sender, EventArgs e)
        {
            if ((((this.Session["e_xid"] != null) && (this.Session["e_xid"].ToString() != "")) && (this.Session["e_mem_id"] != null)) && (this.Session["e_mem_id"].ToString() != ""))
            {
                ...
                int num = this.reg.updateMerchant(this.c_x.xid, this.c_x.xmemberID, this.c_x.xemail, this.c_x.xmobile, this.c_x.xpass, partner.xname, partner.cname);
                StringBuilder builder = new StringBuilder();
                if (num > 0)
                {
        ...
        protected void btnExportExcel_Click(object sender, EventArgs e)
        {
            ...
            this.docpath = docpath + "Merchant-" + this.adminID + "_" + DateTime.Now.ToLongTimeString().Replace(" ", "_").Replace(":", "_") + ".xls";
            this.ef.CreateMemberExcel(this, this.lt_m, this.docpath, "Merchant Details");
        }
        ...
        protected void gvX_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            ...
            if (e.CommandName == "TmDeleteClick")
            {
                namingContainer = (GridViewRow) ((ImageButton) e.CommandSource).NamingContainer;
                rowIndex = namingContainer.RowIndex;
                int num2 = this.reg.updateDeleteXPartner(e.CommandArgument.ToString());
                StringBuilder builder = new StringBuilder();
                if (num2 > 0)
                {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!base.IsPostBack)
            {
                ...
                this.lt_x = this.ret.getPwalletByMemberType("rp");
                ...
                if (this.lt_x.Count > 0)
                {
                    foreach (XObjs.Pwallet pwallet in this.lt_x)
                    {
                        ...
                        XObjs.XPartner partner = new XObjs.XPartner();
                        partner = this.ret.getPartnerByID(item.xmemberID);
                        item.xname = partner.xname;
                        item.cname = partner.cname;
                        item.xpassword = pwallet.xpass;
                        item.nationality = partner.nationality;
                        item.addressID = partner.addressID;
                        item.sys_ID = partner.sys_ID;
                        item.xvisible = partner.xvisible;
                        item.xsync = partner.xsync;
                        item.xreg_date = partner.xreg_date;
                        if (item.xvisible == "1")
                        {
                            this.lt_m.Add(item);
                        }
